...
class RingTest < Minitest::Test
  def test_create_with_ring
    ring = Models::Ring.new({ ring: 'eJwzMDA0ACEjAwNjAA7IAkg=' })
  ...
  def test_create_with_matchids_index
    ring = Models::Ring.new({ match_ids: [1, 2, 3], index: 1 })
    assert_equal 'eJwzMDA0ACEjAwNjAA7IAkg=', ring.to_s
  ...
  def test_create_with_matchids_matchid
    ring = Models::Ring.new({ match_ids: [1, 2, 3], match_id: 2 })
    assert_equal 'eJwzMDA0ACEjAwNjAA7IAkg=', ring.to_s
  ...
  def test_create_with_matchids_no_matchid_no_index
    assert_raises(ArgumentError) { Models::Ring.new({ match_ids: [1, 2, 3] }) }
  end

  def test_create_with_matchids_str
    assert_raises(ArgumentError) do
      Models::Ring.new({ match_ids: '1, 2, 3', match_id: 2 })
    end
  end

  def test_create_no_matchids_no_ring
    assert_raises(ArgumentError) { Models::Ring.new({ match_id: 2 }) }
  end

  def test_create_with_matchids_out_of_order
    ring = Models::Ring.new({ match_ids: [1, 3, 2], match_id: 3 })
  ...
  def test_create_with_matchids_into_hex
    ring = Models::Ring.new({ match_ids: [11, 23, 270], match_id: 23 })
  ...
  def test_create_at_start
    ring = Models::Ring.new({ match_ids: [1, 2, 3], match_id: 1 })
  ...
  def test_create_at_end
    ring = Models::Ring.new({ match_ids: [1, 2, 3], match_id: 3 })
